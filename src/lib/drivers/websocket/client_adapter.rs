...
pub struct WebSocketClientAdapter<C> {
    inner: WebSocketStream<MaybeTlsStream<tokio::net::TcpStream>>,
    codec: C,
    buffer: BytesMut,
}

impl<C> WebSocketClientAdapter<C> {
    pub fn new(
...
impl<C> Sink<Packet> for WebSocketClientAdapter<C>
where
...
impl<C> Stream for WebSocketClientAdapter<C>
where
