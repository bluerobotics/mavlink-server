use std::{net::SocketAddr, sync::Arc};

use anyhow::{Result, anyhow};
use axum::{
    Router,
    extract::{WebSocketUpgrade, ws::WebSocket},
    response::Response,
    routing::get,
};
use futures::StreamExt;
use mavlink_codec::codec::MavlinkCodec;
use tokio::{
    net::TcpListener,
    sync::{RwLock, broadcast},
};
use tower::ServiceBuilder;
use tower_http::{cors::CorsLayer, trace::TraceLayer};
use tracing::*;
use uuid::Uuid;

use crate::{
    callbacks::{Callbacks, MessageCallback},
    drivers::{
        Direction, Driver, DriverInfo,
        generic_tasks::{SendReceiveContext, default_send_receive_run},
        websocket::server_adapter::WebSocketServerAdapter,
    },
    protocol::Protocol,
    stats::{
        accumulated::driver::{AccumulatedDriverStats, AccumulatedDriverStatsProvider},
        driver::DriverUuid,
    },
};

#[derive(Debug)]
pub struct WebSocketServerDriver {
    pub local_addr: String,
    name: arc_swap::ArcSwap<String>,
    uuid: DriverUuid,
    scheme: String,
    direction: Direction,
    on_message_input: Callbacks<Arc<Protocol>>,
    on_message_output: Callbacks<Arc<Protocol>>,
    stats: Arc<RwLock<AccumulatedDriverStats>>,
}

pub struct WebSocketServerDriverBuilder(WebSocketServerDriver);

impl WebSocketServerDriverBuilder {
    pub fn build(self) -> WebSocketServerDriver {
        self.0
    }

    pub fn scheme(mut self, scheme: &str) -> Self {
        self.0.scheme = scheme.to_string();
        self
    }

    pub fn direction(mut self, direction: Direction) -> Self {
        self.0.direction = direction;
        self
    }

    pub fn on_message_input<C>(self, callback: C) -> Self
    where
        C: MessageCallback<Arc<Protocol>>,
    {
        self.0.on_message_input.add_callback(callback.into_boxed());
        self
    }

    pub fn on_message_output<C>(self, callback: C) -> Self
    where
        C: MessageCallback<Arc<Protocol>>,
    {
        self.0.on_message_output.add_callback(callback.into_boxed());
        self
    }
}

impl WebSocketServerDriver {
    #[instrument(level = "debug")]
    pub fn builder(name: &str, local_addr: &str) -> WebSocketServerDriverBuilder {
        let name = Arc::new(name.to_string());

        WebSocketServerDriverBuilder(Self {
            local_addr: local_addr.to_string(),
            name: arc_swap::ArcSwap::new(name.clone()),
            uuid: Self::generate_uuid(&format!("websocket:{local_addr}")),
            scheme: "ws".to_string(),
            direction: Direction::Both,
            on_message_input: Callbacks::default(),
            on_message_output: Callbacks::default(),
            stats: Arc::new(RwLock::new(AccumulatedDriverStats::new(
                name,
                &WebSocketServerInfo,
            ))),
        })
    }

    #[instrument(level = "debug", skip(socket, context))]
    async fn handle_client(
        socket: WebSocket,
        context: &SendReceiveContext,
        identifier: &str,
    ) -> Result<()> {
        debug!("New WebSocket client");

        let codec = MavlinkCodec::<true, true, false, false, false, false>::default();
        let (writer, reader) = WebSocketServerAdapter::new(socket, codec).split();

        if let Err(reason) = default_send_receive_run(writer, reader, identifier, &context).await {
            warn!("Driver send/receive tasks closed: {reason:?}");
        }

        debug!("WebSocket Client connection terminated");

        Ok(())
    }
}

#[async_trait::async_trait]
impl Driver for WebSocketServerDriver {
    #[instrument(level = "debug", skip(self, hub_sender))]
    async fn run(&self, hub_sender: broadcast::Sender<Arc<Protocol>>) -> Result<()> {
        let local_addr = self.local_addr.parse::<SocketAddr>()?;

        let context = SendReceiveContext {
            direction: self.direction,
            hub_sender,
            on_message_output: self.on_message_output.clone(),
            on_message_input: self.on_message_input.clone(),
            stats: self.stats.clone(),
        };

        let mut interval = tokio::time::interval(tokio::time::Duration::from_secs(1));
        let mut first = true;
        loop {
            if first {
                first = false;
            } else {
                interval.tick().await;
            }

            debug!("Trying to bind to address {local_addr:?}...");

            let listener = match TcpListener::bind(&local_addr).await {
                Ok(listener) => listener,
                Err(error) => {
                    error!(
                        "Failed to bind WebSocket to {:?}: {error:?}",
                        self.local_addr
                    );

                    return Err(anyhow!("Failed to bind WebSocket"));
                }
            };

            info!("WebSocket server listening on {local_addr:?}");

            async fn websocket_handler(
                ws: WebSocketUpgrade,
                context: SendReceiveContext,
            ) -> Response {
                ws.on_upgrade(move |socket| {
                    let identifier = Uuid::new_v4();
                    debug!("WS client connected with ID: {identifier}");

                    async move {
                        if let Err(error) = WebSocketServerDriver::handle_client(
                            socket,
                            &context,
                            &identifier.to_string(),
                        )
                        .await
                        {
                            warn!("WebSocket connection {identifier} closed: {error:?}");
                        }
                        debug!("WS client {identifier} removed");
                    }
                })
            }

            let app = Router::new()
                .route(
                    "/",
                    get({
                        let context = context.clone();
                        move |ws: WebSocketUpgrade| websocket_handler(ws, context)
                    }),
                )
                .layer(ServiceBuilder::new())
                .layer(CorsLayer::permissive())
                .layer(TraceLayer::new_for_http());

            if let Err(error) = axum::serve(listener, app).await {
                error!("WebSocket Axum server failed with: {error:?}");
                continue;
            }
        }
    }

    #[instrument(level = "debug", skip(self))]
    fn info(&self) -> Box<dyn DriverInfo> {
        Box::new(WebSocketServerInfo)
    }

    fn name(&self) -> Arc<String> {
        self.name.load_full()
    }

    fn uuid(&self) -> &DriverUuid {
        &self.uuid
    }
}

#[async_trait::async_trait]
impl AccumulatedDriverStatsProvider for WebSocketServerDriver {
    async fn stats(&self) -> AccumulatedDriverStats {
        self.stats.read().await.clone()
    }

    async fn reset_stats(&self) {
        let mut stats = self.stats.write().await;
        stats.stats.input = None;
        stats.stats.output = None
    }
}

pub struct WebSocketServerInfo;
impl DriverInfo for WebSocketServerInfo {
    fn name(&self) -> &'static str {
        "WsServer"
    }
    fn valid_schemes(&self) -> &'static [&'static str] {
        &["wsserver", "wssserver"]
    }

    fn cli_example_legacy(&self) -> Vec<String> {
        let first_schema = self.valid_schemes()[0];

        vec![
            format!("{first_schema}:<HOST>:<PORT>"),
            format!("{first_schema}:0.0.0.0:9988"),
            format!("{first_schema}:192.168.1.100:9000"),
        ]
    }

    fn cli_example_url(&self) -> Vec<String> {
        let first_schema = &self.valid_schemes()[0];
        vec![
            format!("{first_schema}://<HOST>:<PORT>?direction=<DIRECTION|receiver,sender>"),
            url::Url::parse(&format!("{first_schema}://0.0.0.0:9988"))
                .unwrap()
                .to_string(),
            url::Url::parse(&format!(
                "{first_schema}://192.168.1.100:9000?direction=receiver"
            ))
            .unwrap()
            .to_string(),
        ]
    }

    fn create_endpoint_from_url(&self, url: &url::Url) -> Option<Arc<dyn Driver>> {
        let scheme = url.scheme().strip_suffix("server").unwrap();
        let host = url.host_str().unwrap_or("0.0.0.0");
        let port = url.port().unwrap_or(9988);
        let local_addr = format!("{host}:{port}");

        let direction = url
            .query_pairs()
            .find_map(|(key, value)| {
                if key != "direction" {
                    return None;
                }
                value.parse().ok()
            })
            .unwrap_or(Direction::Both);

        Some(Arc::new(
            WebSocketServerDriver::builder("WsServer", &local_addr)
                .scheme(scheme)
                .direction(direction)
                .build(),
        ))
    }
}
