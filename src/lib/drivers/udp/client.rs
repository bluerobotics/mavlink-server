...
impl Driver for UdpClient {
    ...
    async fn run(&self, hub_sender: broadcast::Sender<Arc<Protocol>>) -> Result<()> {
        ...
        loop {
            ...
            if let Err(reason) = udp_send_receive_run(
                writer,
                reader,
                &remote_addr,
                &hub_sender,
                &mut hub_receiver,
                &self.on_message_output,
                &self.on_message_input,
                self.stats.clone(),
            )
            .await
            {
...
{
    tokio::select! {
        result = udp_send_task(&mut writer, remote_addr, hub_receiver, on_message_output, &stats) => {
            if let Err(error) = result {
                error!("Error in send task for {remote_addr}: {error:?}");
            }
        }
        result = udp_receive_task(&mut reader, remote_addr, hub_sender, on_message_input, &stats) => {
            if let Err(error) = result {
    ...
    S: Sink<(Packet, SocketAddr), Error = std::io::Error> + std::marker::Unpin,
{
    loop {
        ...
        for future in on_message_output.call_all(message.clone()) {
            if let Err(error) = future.await {
                debug!(remote = ?remote_addr, "Dropping message: on_message_output callback returned error: {error:?}");
                continue;
            }
        }
        ...
        if let Err(io_error) = writer.send(((**message).clone(), *remote_addr)).await {
            ...
            trace!(remote = ?remote_addr, "Failed to send message: {io_error:?}");
            break;
        }
